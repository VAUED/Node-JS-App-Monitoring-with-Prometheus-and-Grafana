...
const createDelayHandler = async (req, res) => {
  if ((Math.floor(Math.random() * 100)) === 0) {
    ...
    httpRequestsErrorGauge.inc({ method: req.method, route: req.path, code: '500' });
...
app.get('/metrics', async (req, res) => {
  ...
  httpRequestsTotalGauge.inc({ method: req.method, route: route, code: res.statusCode });
});
...
app.get('/slow', async (req, res) => {
  ...
  if (res.statusCode === 500) {
    httpRequestsError.inc({ method: req.method, route: route, code: res.statusCode });
    httpRequestsErrorGauge.inc({ method: req.method, route: route, code: res.statusCode });
  }
  httpRequestsTotal.inc({ method: req.method, route: route, code: res.statusCode });
  httpRequestsTotalGauge.inc({ method: req.method, route: route, code: res.statusCode });
 });
...
app.get('/', (req, res) => {
    ...
    httpRequestsTotalGauge.inc({ method: req.method, route: req.path, code: '200' });
